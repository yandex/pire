...
Скомпилированная регулярка хранится в сканере (т.е. конечном автомате, представленном
...
Final() имеет смысл вызвать AcceptedRegexps(const State&), возвращающую пару
...
Scanner::Load() можно воспользоваться Scanner::Mmap(), который не будет выполнять
...
При чтении закрывающей круглой скобки фрагмент автомата, соответствующий
