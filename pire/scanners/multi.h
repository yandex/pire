...
class ScannerStateRegexpHash {
public:
	ScannerStateRegexpHash(const Scanner& scanner)
		: m_scanner(scanner)
	{}

	size_t operator()(const size_t& state) const {
		const auto regexps = m_scanner.AcceptedRegexps(state);
		size_t result = 0;
		for (auto iter = regexps.first; iter != regexps.second; ++iter) {
			result ^= std::hash<size_t>{}(*iter) + 0x9e3779b9 + (result<<6) + (result>>2);
		}
		return result;
	}
...
template <typename Scanner>
class ScannerStateRegexpEquality {
public:
	ScannerStateRegexpEquality(const Scanner& scanner)
		: m_scanner(scanner)
	{}

	bool operator()(const size_t& state1, const size_t& state2) const {
		const auto regexps1 = m_scanner.AcceptedRegexps(state1);
		const auto regexps2 = m_scanner.AcceptedRegexps(state2);
		auto iter1 = regexps1.first;
		auto iter2 = regexps2.first;
		while (iter1 != regexps1.second && iter2 != regexps2.second) {
			if (*iter1 != *iter2) {
				return false;
			}
			++iter1;
		...
		return iter1 == regexps1.second && iter2 == regexps2.second;
	}
...
class ScannerLettersEquality {
public:
	ScannerLettersEquality(const Scanner& scanner)
		: m_letters(scanner.m_letters)
	{}

	bool operator()(Char a, Char b) {
		return m_letters[a] == m_letters[b];
	}
private:
	const typename Scanner::Letter* m_letters;
};

template<class Scanner>
class ScannerMinimizeTask {
public:
	typedef Partition<Char, ScannerLettersEquality<Scanner>> LettersTbl;
	typedef THashMap<size_t, size_t, ScannerStateRegexpHash<Scanner>, ScannerStateRegexpEquality<Scanner>> StateClassesHashMap;

	ScannerMinimizeTask(const Scanner& scanner)
		: m_scanner(scanner)
		, m_letters(ScannerLettersEquality<Scanner>(m_scanner))
		, reversedTransitions(scanner.Size())
		, invletters(0)
		, StateClass(scanner.Size())
		, Classes(0)
	{
		...
		invletters.reserve(m_letters.Size());
		for (const auto& letter : m_letters) {
			invletters.push_back(letter.first);
		}

		StateClassesHashMap stateClassMap(m_scanner.RegexpsCount() * 2, ScannerStateRegexpHash<Scanner>(m_scanner), ScannerStateRegexpEquality<Scanner>(m_scanner));
		for (size_t stateId = 0; stateId < m_scanner.Size(); ++stateId) {
			const auto state = m_scanner.IndexToState(stateId);
			if (stateClassMap.find(state) == stateClassMap.end()) {
				stateClassMap[state] = Classes++;
			}
			StateClass[stateId] = stateClassMap[state];
			reversedTransitions[stateId].resize(m_letters.Size());
		}


		for (size_t stateId = 0; stateId < Size(); ++stateId) {
			for (size_t letterId = 0; letterId < m_letters.Size(); ++letterId) {
				auto destination = m_scanner.IndexToState(stateId);
				m_scanner.Next(destination, invletters[letterId]);
				reversedTransitions[m_scanner.StateIndex(destination)][letterId].push_back(stateId);
			}
		}
	}
	...
	size_t LettersCount() const { return m_letters.Size(); }

	bool IsDetermined() const { return true; }

	size_t Size() const { return m_scanner.Size(); }
	...
	void AcceptStates() {
		size_t finalTableSize = 0;
		TVector<size_t> classState(Classes, Size());
		for (size_t stateId = 0; stateId < Size(); ++stateId) {
			if (classState[StateClass[stateId]] == Size()) {
				classState[StateClass[stateId]] = stateId;
				finalTableSize += m_scanner.AcceptedRegexpsCount(stateId);
			}
		}

		this->SetSc(std::unique_ptr<Scanner>(new Scanner()));
		Sc().Init(Classes, m_letters, finalTableSize, StateClass[m_scanner.StateIndex(m_scanner.m.initial)], m_scanner.RegexpsCount());

		for (size_t classId = 0; classId < Classes; ++classId) {
			const auto oldState = m_scanner.IndexToState(classState[classId]);
			Sc().m_finalIndex[classId] = Sc().m_finalEnd - Sc().m_final;
			Sc().m_finalEnd = ScannerGlueTask<Scanner>::Shift(m_scanner.AcceptedRegexps(oldState), 0, Sc().m_finalEnd);
			*Sc().m_finalEnd++ = static_cast<size_t>(-1);

			Sc().SetTag(classId, (m_scanner.Final(oldState) ? Scanner::FinalFlag : 0)
				| (m_scanner.Dead(oldState) ? Scanner::DeadFlag : 0));

			for (const auto& letter : invletters) {
				auto destination = oldState;
				m_scanner.Next(destination, letter);
				Sc().SetJump(classId, letter, StateClass[m_scanner.StateIndex(destination)]);
			}
		}
	}
...
Impl::Scanner<Relocation, Shortcutting> Impl::Scanner<Relocation, Shortcutting>::Glue(const Impl::Scanner<Relocation, Shortcutting>& lhs, const Impl::Scanner<Relocation, Shortcutting>& rhs, size_t maxSize /* = 0 */)
{
	...
	Impl::Scanner<Relocation, Shortcutting> notMinimized = Impl::Determine(task, maxSize ? maxSize : DefMaxSize);
	Impl::ScannerMinimizeTask< Impl::Scanner<Relocation, Shortcutting> > minimizeTask(notMinimized);
	return Impl::Minimize(minimizeTask);
}
